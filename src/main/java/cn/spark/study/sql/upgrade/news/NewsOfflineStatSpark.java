...
public class NewsOfflineStatSpark {
	...
	public static void main(String[] args) {
		...
		SparkSession hiveContext= SparkSession.builder().config(conf).getOrCreate();
...
//		HiveContext hiveContext = new HiveContext(sc.sc());
	...
	private static void calculateDailyPagePv(
			SparkSession hiveContext, String date) {
		String sql = 
				"SELECT "
					+ "date,"
					+ "pageid,"
	...
	private static void calculateDailyNewUserRegisterRate(
		...
		// 执行两条SQL，获取结果
		Object result1 = hiveContext.sql(sql1).collectAsList().get(0);
		...
		Object result2 = hiveContext.sql(sql2).collectAsList().get(0);
		long number2 = 0L;
		if(result2 != null) {
